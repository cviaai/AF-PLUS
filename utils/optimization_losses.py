...
def image_gradient_x(img, kernel_name='default'):
    ...
    conv1=nn.Conv2d(1, 1, kernel_size=kernel_vert.shape, stride=1,
...
def image_gradient_y(img, kernel_name='default'):
    ...
    conv1=nn.Conv2d(1, 1, kernel_size=kernel_horiz.shape, stride=1,
                    padding=1, bias=False)
...
def tsavgol(signal, kernel=6, stride=2, N=3):
    ...
    patches = torch.ones_like(signal).unfold(0, kernel, stride) 
    m = F.fold(patches.permute(1,0)[None], output_size=(signal.shape[-1], 1), 
               kernel_size=(kernel, 1), stride=(stride, 1))[0, 0, :, 0]
    ...
    return F.fold(new_patches.permute(1,0)[None], 
                  output_size=(signal.shape[0], 1),
...
def savitzky_golay(rot_vector, window_size=11, order=3, deriv=0, rate=1):
    ...
    bb = torch.vander(torch.arange(-half_window, half_window+1), N=order+1,
                      increasing=True).float().to(rot_vector.device)

    kernel = (torch.linalg.pinv(bb)[deriv] * rate** deriv * \
              factorial(deriv)).float().to(rot_vector.device)
    
    # pad the signal at the extremes with values taken from the signal itself
    firstvals = rot_vector[0] - \
    torch.abs(torch.flip(rot_vector[1:half_window+1], (0, )) - rot_vector[0])
    lastvals = rot_vector[-1] + \
    torch.abs(torch.flip(rot_vector[-half_window-1:-1], (0, )) - rot_vector[-1])
    
    rot_vector = torch.cat([firstvals, rot_vector, lastvals])
    
    conv=nn.Conv1d(1, 1, kernel_size=kernel.shape, stride=1, padding=0,
                   bias=False)
...
def moving_average(rot_vector, window_size=11):
    ...
    conv=nn.Conv1d(1, 1, kernel_size=window_size, stride=1, 
...
def median_filter(rot_vector, window_size=11):
    ...
    torch.abs(torch.flip(rot_vector[1:half_window+1], (0, )) - rot_vector[0])
    lastvals = rot_vector[-1] + \
    torch.abs(torch.flip(rot_vector[-half_window-1:-1], (0, )) \
              - rot_vector[-1])
    rot_vector = torch.cat([firstvals, rot_vector, lastvals])

    for elem in range(rot_vector_shape):
        median_vector[elem] = torch.median(rot_vector[elem : elem + \
                                                      window_size])
